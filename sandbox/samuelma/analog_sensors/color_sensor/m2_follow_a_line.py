...
def follow_the_line(robot, white_level, black_level):
    ...
    """
    robot.touch_sensor = ev3.TouchSensor()
    ...
    while(True):
        if robot.touch_sensor.is_pressed == 1:
            break
        if abs(robot.color_sensor.reflected_light_intensity - black_level) <= 5:
            robot.drive_until_otherwise(100, 100)
        elif abs(robot.color_sensor.reflected_light_intensity - white_level) <= 5:
            robot.drive_until_otherwise(-100, 100)
